<?php
/**
 * One-shot: add (or update) the 'hint' field on the EXISTING /review/ gate key
 * file, without touching its hash or signing secret. Same install shape as
 * _setup-gate.php (VPS-only, POST-only, reachable only via a temporary
 * .htaccess exemption), but this one never rotates the password -- it merges
 * a single new field into whatever is already there and refuses to run if the
 ...
 *     https://boubacarbarry.com/review/_add-hint.php
 *
 * Runbook: uncomment the RewriteRule exemption for this file in
 * review/.htaccess, deploy, run the curl above, then comment the exemption
 * back out and deploy again. Leaving this reachable is a standing invitation
 * for anyone who finds the address to rewrite the hint text (never the
 * password -- this script cannot touch hash or secret).
 */
...
// Anchored on __DIR__, exactly as gate.php is, so this and the gate can never
// disagree about where the key lives.
$keyPath = dirname(__DIR__, 2) . '/.review-gate-key.php';
...
$body = "<?php\n"
      . "// /review/ gate key. Written by gate.php's installer + _add-hint.php. Never commit this.\n"
